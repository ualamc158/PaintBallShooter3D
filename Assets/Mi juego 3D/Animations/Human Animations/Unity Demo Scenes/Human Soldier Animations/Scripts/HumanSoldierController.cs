...
public class HumanSoldierController : MonoBehaviour
{
    ...
    private void AtacarObjetivo()
    {
        ...
        // 2. Rotar para mirar al jugador (suavizado para que no gire de golpe)
        ...
        if (estadoActual != EstadoEnemigo.Atacando)
        {
            ...
            // He quitado el "return;" aquí para que pueda disparar inmediatamente si es necesario
        }

        // 4. Lógica de Disparo
        if (arma == null)
        {
            Debug.LogWarning("¡El enemigo no tiene el script ControlArma asignado o no se encontró!");
            return;
        }

        if (arma.PuedeDisparar())
        {
            Debug.Log("¡El arma puede disparar! Enviando el Trigger 'Shoot01' al Animator...");
            arma.Disparar();
            ...
            if (anim != null)
            {
                anim.ResetTrigger("Run");
                anim.ResetTrigger("NoMovement");
                anim.SetTrigger("Shoot01"); // <-- El Trigger exacto
            }
        }
    }
